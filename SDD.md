...
- `core/database.py`：SQLAlchemy 会话、建表、SQLite 外键启用，以及启动期旧库迁移（先补列：`officials.party_role`、履历刷新跟踪列 `resume_hash`/`resume_refreshed_at`；再做带 ORM 查询的标签回填与页面权限键别名合并去重）。
- `api/officials.py`：履历 CRUD、统计、机构、批量时间线、人物关系与履历刷新 API；列表接口支持 `status` 与 `party_role` 筛选，党内职务按“常委 ⊂ 政治局委员 ⊂ 中央委员”层级展开匹配，未知取值返回 400。`POST /api/officials/resume-refresh` 创建 `resume_refresh` 类型后台任务并立即返回运行 ID，存在运行中的同类型任务时返回 409。
- `services/official/refresh.py`：履历刷新任务。逐个官员通过 WebFetch 抓取 `source_url`（维基百科链接自动使用服务的代理策略出网），HTML 归一化为纯文本后计算 SHA-256：增量模式下哈希未变化直接跳过；否则交由专用解析器抽取结构化任职经历，整体替换该官员 `careers` 并记录哈希与刷新时间。单人失败回滚并记录 ERROR 日志后继续；解析结果为空视为失败且不落哈希（下次自动重试）；全部官员失败时任务整体失败。
- `services/official/resume_parsers/`：履历专用解析器（不依赖 LLM）。`dateparse.py` 为共享的中文履历日期区间引擎（适配自种子数据生成脚本 `scripts/ccdata/extract_proto.py`，支持 1973－1975年／2018年5月24日—2023年7月4日／至今 等写法，含时间链补全、身份噪音过滤）；`wikipedia.py` 为维基百科条目 DOM 专用解析器（“生平/履历”章节优先，infobox“职务+任期”兜底，导语最后）；`govmedia.py` 为官媒标准简历格式（日期行）通用解析器，作为非维基来源的默认解析器。
...
- `officials`：人物基本履历，含党内职务 `party_role`（中央政治局常委/中央政治局委员/中央委员/中央候补委员，空表示无）。旧库升级时自动补列；每次启动对空值行按标签回填（识别中央委员、候补委员、政治局常委/委员标签），不覆盖已有值。履历刷新跟踪列 `resume_hash`（上次抓取文本 SHA-256，供增量去重）与 `resume_refreshed_at`（上次刷新时间）同样在旧库升级时自动补列。
...
WebFetch 客户端只调用配置的集中抓取服务；API Key 通过请求头传递，不写入日志。维基百科域名需在抓取服务侧加入域名白名单（`WEBFETCH_SECURITY__ALLOWED_HOSTS`，国内 DNS 污染会导致其解析 IP 被当作 SSRF 目标拒绝），并由刷新任务按请求指定 `proxy_policy=proxy` 经服务代理出网；官媒等国内域名保持直连。CORS 当前允许同源部署兼容的全来源访问，若拆分部署应在配置层收紧来源。

## 6. 运行与部署

启动脚本读取配置端口、创建 `.venv`、安装后端依赖、构建前端、创建数据和日志目录并启动 Uvicorn。状态脚本同时校验 PID 和健康接口。systemd 模板调用 `run.sh` 前台运行。

Jenkins 流程：检出主分支 → 后端测试 → 前端构建 → 停止旧服务 → 写入提交编号到 `config/version.txt` 并 rsync `src_gpt5.6` → 首次创建密码文件 → 启动 → 状态与健康检查。增量部署排除 `data`、`logs`、虚拟环境、缓存、`node_modules` 和 `config/env.local`。`config/version.txt` 缺失时运行时版本回退为 `0.1.0`。

## 7. 测试设计

- 单元测试：配置、安全、时间、适配器、分析引擎、信息源、履历 CRUD、人物关系、履历专用解析器（日期区间引擎、维基章节/信息框双形态、官媒标准格式、噪音过滤）、履历刷新任务（增量跳过、full 重刷、代理策略路由、单人失败隔离、空解析不落哈希、并发任务 409、迁移补列）。
- 冒烟测试：登录、信息源、分析任务、运行结果、页面权限键迁移和健康接口（含版本号）。
