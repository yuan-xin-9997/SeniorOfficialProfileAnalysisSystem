...
- `services/official/refresh.py`：履历刷新任务。逐个官员通过 WebFetch 抓取 `source_url`，HTML 归一化为纯文本后计算 SHA-256：增量模式下哈希未变化直接跳过；否则调用 LLM 抽取结构化任职经历（JSON），整体替换该官员 `careers` 并记录哈希与刷新时间。单人失败回滚并记录 ERROR 日志后继续；解析结果为空视为失败且不落哈希（下次自动重试）；全部官员失败时任务整体失败。
...
- 单元测试：配置、安全、时间、适配器、分析引擎、信息源、履历 CRUD、人物关系、履历刷新任务（增量跳过、full 重刷、单人失败隔离、空解析不落哈希、并发任务 409、迁移补列）。
