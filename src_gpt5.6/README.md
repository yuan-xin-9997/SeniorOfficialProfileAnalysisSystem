...
- 概览：核心统计、最近履历、数据完整度。
- 履历档案：人物分页检索、按状态（在任/离任/退休/落马/已故）与党内职务（中央政治局常委/中央政治局委员/中央委员/中央候补委员，层级语义）筛选、编辑、详情与任职时间轴；一键「履历刷新」创建后台任务，通过 WebFetch 增量抓取全部官员来源页面并用专用解析器更新结构化履历（不调用 LLM）。解析器按来源自动适配：维基百科走 DOM 专用解析器（生平章节优先、信息框兜底）并经抓取服务代理出网，官媒等标准简历格式走通用日期行解析器；新增官员把来源链接配成上述页面即可自动适配。页面内容未变化自动跳过，单个官员解析失败不影响其他人，解析不出履历时不覆盖原有数据，进度可在任务中心查看。
...
- 任务中心：任务状态、摘要、错误和日志，覆盖采集、分析与履历刷新任务。
