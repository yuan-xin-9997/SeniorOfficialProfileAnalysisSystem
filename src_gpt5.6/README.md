...
- 概览：核心统计、最近履历、数据完整度。
- 履历档案：人物分页检索、按状态（在任/离任/退休/落马/已故）与党内职务（中央政治局常委/中央政治局委员/中央委员/中央候补委员，层级语义）筛选、编辑、详情与任职时间轴；一键「履历刷新」创建后台任务，通过 WebFetch 增量抓取全部官员来源页面并调用 LLM 更新结构化履历（页面内容未变化自动跳过，单个官员解析失败不影响其他人，可在任务中心查看进度）。
