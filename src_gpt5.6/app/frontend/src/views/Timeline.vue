    ...
    <div class="panel timeline-controls">
      <div class="selector-heading">
        ...
        <div class="update-actions">
          <label>纵轴时间
            <select v-model="selectedGranularity" :disabled="loading">
      ...
      <p class="selection-summary">已选择 {{ selectedIds.length }} 人<span v-if="loadedAt"> · 当前按{{ appliedGranularity === 'year' ? '年' : '月' }}展示 · 最近更新 {{ loadedAt }}</span></p>
    </div>
    ...
    <div v-else class="panel timeline-chart-panel">
      <div class="timeline-scroll">
        <div class="timeline-chart" :style="chartStyle">
          <div class="axis-corner">{{ appliedGranularity === 'year' ? '年份' : '年月' }}</div>
          ...
          <div class="axis-column" :style="{ height: `${chartHeight}px` }">
            <div v-for="tick in ticks" :key="tick.index" class="axis-tick" :class="{ year: tick.major }" :style="{ top: `${tick.index * rowHeight}px` }">
              <span>{{ tick.label }}</span>
            </div>
          </div>
          <div v-for="person in timelineOfficials" :key="`lane-${person.id}`" class="person-lane" :style="{ height: `${chartHeight}px` }">
            <div v-for="tick in ticks" :key="tick.index" class="month-line" :class="{ year: tick.major }" :style="{ top: `${tick.index * rowHeight}px` }"></div>
...
const loadedAt = ref('')
const selectedGranularity = ref<'year' | 'month'>('month')
const appliedGranularity = ref<'year' | 'month'>('month')
const rowHeight = computed(() => appliedGranularity.value === 'year' ? 92 : 22)
...
async function updateTimeline() {
  ...
  try {
    ...
    appliedGranularity.value = selectedGranularity.value
...
const startUnit = computed(() => appliedGranularity.value === 'year' ? Math.floor(startMonth.value / 12) : startMonth.value)
const endUnit = computed(() => appliedGranularity.value === 'year' ? Math.floor(endMonth.value / 12) : endMonth.value)
const unitCount = computed(() => Math.max(1, endUnit.value - startUnit.value + 1))
const chartHeight = computed(() => unitCount.value * rowHeight.value)
const ticks = computed(() => Array.from({ length: unitCount.value }, (_, index) => {
  const value = startUnit.value + index
  if (appliedGranularity.value === 'year') return { index, major: true, label: `${value}年` }
  const month = value % 12 + 1
  return { index, major: month === 1, label: month === 1 || index === 0 ? monthLabel(value) : String(month).padStart(2, '0') }
}))
